package cn.keepfight.services.mdService.YamlFrontMatter.internal;

import cn.keepfight.services.mdService.YamlFrontMatter.YamlFrontMatterBlock;
import cn.keepfight.services.mdService.YamlFrontMatter.YamlFrontMatterNode;
...
public class YamlFrontMatterBlockParser extends AbstractBlockParser {
     ...
     * 修改为行内可数组
     */
    private static final Pattern REGEX_METADATA_INLINE_LIST = Pattern.compile("(\\[(.+?,)*.+?\\])\\s*");
        ...
        if (inYAMLBlock) {
            ...
            if (matcher.matches()) {
                ...
                if ("|".equals(matcher.group(2))) {
                    ...
                    Matcher g2Matcher = REGEX_METADATA_INLINE_LIST.matcher(g2);
                    if (g2Matcher.matches()) {
                        System.out.println("matches():true");
                        List<String> ss = Arrays.asList(g2Matcher.group(1)
                                .replace("]", "")
                                .replace("[", "")
                                .split(","));
                        System.out.println("ss:"+ss);
                        currentValues.addAll(ss);
                    } else {
